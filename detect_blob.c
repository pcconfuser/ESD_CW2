...
// -o -  # write to stdout
#define CAMERA_CMD "raspistill -w 200 -h 200 -t 1 -awb fluorescent --nopreview --mode 7 -rot 270" // use if needed: -rot 0/90/180
...
// return status (0 for success)
{
    ...
    for (x=0; x < stream->w; x++) {
        ...
        if (
             ...
             //pixel[0] >= 100 && pixel[1] <= 80 && pixel[2] <= 80  // red color
             pixel[0] <= 80 && pixel[1] <= 80 && pixel[2] >= 100  // blue color
            )
            stream->row[x] = 255;
        else
            stream->row[x] = 0;
